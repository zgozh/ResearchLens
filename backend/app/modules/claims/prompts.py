...
CLAIM_EXTRACTION_PROMPT = (
    ...
    "2. **quote 必须原样复制该块文字**（ADR-0051，实测失败模式）：\n"
    "   - **保持原文语言**：中文块就抄中文，**严禁翻译成英文**（也不要把英文块译成中文）；\n"
    "   - 标点、数字、单位、公式标记照抄，不得增删字词或合并句子；\n"
    "   - 一个 quote 只能来自**一个** block，不要跨块拼接；\n"
    "   - 不要给 quote 加任何前缀（如 \"原文为：\"、\"Fig.x\"）或后缀说明。\n"
    "3. 保留数据集、指标、数字、适用条件（仅在…/假设…）与局限；不得把它们省略成「效果好」。\n"
    "4. 找不到原文依据的结论**不要输出**；不要为了凑数捏造 claim。\n"
    "5. claim_id 用简短英文/数字标识（≤32 字符）。\n"
    "6. 只返回 JSON，quotes 字段必须非空。\n"
)
