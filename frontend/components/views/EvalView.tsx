...
const LABEL: Record<string, string> = {
  ...
  structure_accuracy: 'Structure Extraction Accuracy',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[300px,1fr]">
        <GlassCard className="flex flex-col items-center justify-center p-6 text-center">
          <Kicker className="mb-3">RESEARCHLENS SCORE</Kicker>
          <motion.div
        ...
        <GlassCard className="p-6">
          <Kicker className="mb-4">HEADLINE METRICS</Kicker>
          <div className="grid grid-cols-3 gap-4">
            {[
              { v: m.citation_coverage ?? 0, l: 'Evidence Coverage', c: '#22D3EE' },
              { v: m.claim_evidence_alignment ?? 0, l: 'Claim–Evidence Align.', c: '#6366F1' },
              { v: unsupported, l: 'Unsupported Claims', c: unsupported === 0 ? '#34D399' : '#F59E0B' },
            ].map((x) => (
          ...
          {(m.num_claims != null) && (
            <div className="mt-5 flex items-center justify-between border-t border-[var(--line)] pt-4 text-[12px] text-slate-400">
              <span className="inline-flex items-center gap-2">
                ...
                {m.num_supported}/{m.num_claims} claims grounded
              </span>
              <span className={unsupported === 0 ? 'inline-flex items-center gap-1.5 text-emerald-400' : 'inline-flex items-center gap-1.5 text-amber-400'}>
                ...
                {unsupported === 0 ? '0 unsupported claims (target ≈ 0)' : `${unsupported}% unsupported`}
              </span>
            </div>
          )}
        </GlassCard>
      ...
      {/* metric meters */}
      <GlassCard className="p-6">
        <Kicker className="mb-4">EVALUATION METRICS</Kicker>
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          <div>
            <Meter
              value={unsupported}
              label="Unsupported Claim Rate (目标 ≈ 0)"
      ...
      <GlassCard className="border-dashed p-5 text-center font-mono text-[12px] text-slate-500">
        ResearchLens Evaluation — 指标基于论文内部真实的断言↔证据链接关系测算，非编造数字。
      </GlassCard>
    </div>
  );
}
