  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
      ...
      <div className="lg:col-span-3">
        <Kicker className="mb-3">CLAIMS · 可验证断言</Kicker>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {c.status === 'SUPPORTED' ? (
                        ...
                        <span className="inline-flex items-center gap-1 text-[11px] text-amber-400">
                          <ShieldAlert className="h-3.5 w-3.5" /> unsupported
                        </span>
                      )}
                    </div>
                    <span className="font-mono text-[11px] text-slate-500">conf {c.confidence.toFixed(2)}</span>
                  </div>
                  <p className="mt-2.5 text-sm leading-relaxed text-slate-200">{c.statement}</p>
                  <div className="mt-3 flex items-center gap-3 text-[11px] text-slate-500">
                    <span className="inline-flex items-center gap-1">
                      <FileText className="h-3 w-3" /> {c.evidence_count} evidence
                    </span>
                  </div>
                </button>
              </motion.div>
      ...
      <div className="lg:col-span-2">
        <Kicker className="mb-3">RESULTS · 结果图表</Kicker>
        <div className="space-y-4">
          {(detail.tables || []).map((t) => (
            <GlassCard key={t.table_no} className="overflow-hidden p-4">
              <div className="mb-2 flex items-center gap-2">
                <Table2 className="h-3.5 w-3.5 text-slate-400" />
                <span className="text-xs font-semibold text-slate-200">Table {t.table_no}</span>
