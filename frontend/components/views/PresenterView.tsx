...
import { Play, Pause, SkipForward, SkipBack, FileText, Layers, Table2, Quote } from 'lucide-react';
...
export function PresenterView({ presentation, accent, detail, claims }: {
  ...
  useEffect(() => {
    if (!playing) return;
    const id = setTimeout(() => (idx + 1 < scenes.length ? setIdx(idx + 1) : setPlaying(false)), 6000);
      ...
      <GlassCard className="flex flex-col overflow-hidden">
        <div className="flex items-center justify-between border-b border-[var(--line)] px-5 py-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Btn variant="ghost" className="h-8 w-8 p-0" onClick={() => { goto(idx - 1); setPlaying(false); }}><SkipBack className="h-4 w-4" /></Btn>
            <Btn variant="ghost" className="h-8 w-8 p-0" onClick={() => { goto(idx + 1); setPlaying(false); }}><SkipForward className="h-4 w-4" /></Btn>
          </div>
        </div>
        ...
        <GlassCard className="sticky top-4 max-h-[calc(100vh-6rem)] overflow-y-auto p-5">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={sel ? JSON.stringify(sel) : 'empty'} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              {!sel && <p className="text-[13px] text-slate-500">点击左侧「涉及内容」的图/表/原文，这里会展示对应证据。</p>}
              {sel?.type === 'figure' && (() => {
